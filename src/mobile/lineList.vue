<template>
  <div>
    ...
    <div v-infinite-scroll="loadMore" infinite-scroll-distance="10" infinite-scroll-disabled="loading">
      <mt-cell v-for="(item, index) in line" :key="index" :title="item.name" :is-link="true" :to="{path: '/mobile/main/line/detail/' + item.id + '/' + item.state + '/' + (item.serials === '' ? 'none' : item.serials) + '/' + (item.batch === '' ? 'none' : item.batch) + '/' + item.startTime + '/' + item.timelenth}">
